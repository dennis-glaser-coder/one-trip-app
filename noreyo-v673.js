/* NOREYO V6.73 — search request ownership across SPA navigation/BFCache.
...
function installFetchHook(){
  try{
    if(typeof window.fetch!=='function'||window.fetch.__noreyoV673)return;
    ...
    wrapped.__noreyoV673=true;
    window.fetch=wrapped;
  }catch(_){ }
}
function navigationAway(view){
  return ['favorites','trips','profile','discover','searchView'].includes(String(view||''));
}
function installGoHook(){
  try{
    if(typeof go!=='function'||go.__noreyoV673)return;
    const prior=go;
...
window.NOREYO_V673=Object.freeze({
