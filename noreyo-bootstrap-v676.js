/* NOREYO V6.76 bootstrap — compact versioned outer loader. */
(()=>{
'use strict';
const BUILD='6.76-safe';
const SITE_BUILD='noreyo-676';
const HOTFIX=`
...
const SCRIPT_ASSETS=[
 ['noreyo-v533.js',643],['noreyo-v541.js',663],['noreyo-v544.js',657],['noreyo-v546.js',666],
...
async function unzipNative(buffer){
  const u8=new Uint8Array(buffer),dv=new DataView(buffer),td=new TextDecoder(),out={};let off=0;
  while(off+30<=u8.length&&dv.getUint32(off,true)===0x04034b50){
    const flags=dv.getUint16(off+6,true),method=dv.getUint16(off+8,true),csize=dv.getUint32(off+18,true);
    const nlen=dv.getUint16(off+26,true),xlen=dv.getUint16(off+28,true);
    if(flags&0x08)throw new Error('ZIP data descriptor unsupported');
    const name=td.decode(u8.subarray(off+30,off+30+nlen)),start=off+30+nlen+xlen,comp=u8.slice(start,start+csize);
    let raw;if(method===0)raw=comp;else if(method===8)raw=await inflateRaw(comp);else throw new Error('ZIP compression '+method+' unsupported');
    out[name]=raw;off=start+csize;
  }
  if(!out['index.html'])throw new Error('index.html fehlt im Paket');
  return out;
}
function loadScript(src,timeoutMs=15000){
  return new Promise((resolve,reject)=>{
    ...
    const finish=(ok,error)=>{
      if(done)return;done=true;clearTimeout(timer);s.onload=s.onerror=null;
      if(ok)resolve();else{try{s.remove();}catch(_){ }reject(error||new Error('Script konnte nicht geladen werden'));}
    };
    const timer=setTimeout(()=>finish(false,new Error('Script-Timeout: '+src)),timeoutMs);
    s.src=src;s.onload=()=>finish(true);s.onerror=()=>finish(false,new Error('Script konnte nicht geladen werden: '+src));
...
async function fetchWithTimeout(url,init={},timeoutMs=30000){
  ...
  catch(error){if(controller.signal.aborted)throw new Error('Netzwerk-Timeout: '+url);throw error;}
  finally{clearTimeout(timer);}
}
async function unzipFallback(buffer){
  setStatus('Kompatibilitätsmodus wird geladen …');
  await loadScript('https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js');
  const z=await JSZip.loadAsync(buffer),out={};
  for(const [name,e] of Object.entries(z.files))if(!e.dir)out[name]=new Uint8Array(await e.async('uint8array'));
  return out;
}
function brand(html){
  html=html.split('ONE TRIP').join('NOREYO').split('One Trip').join('NOREYO').split('one trip').join('NOREYO').split('zeigt dir zuerst die Reisen').join('zeigt dir die Reisen');
  html=html.replace(/<title>.*?<\/title>/i,'<title>NOREYO – Travel made for you.</title>');
  html=html.replace(/<meta\s+name="application-name"[^>]*>/i,'<meta name="application-name" content="NOREYO">');
  if(!/name="apple-mobile-web-app-title"/i.test(html))html=html.replace('</title>','</title>\n<meta name="apple-mobile-web-app-title" content="NOREYO">');
  if(!/name="description"/i.test(html))html=html.replace('</title>','</title>\n<meta name="description" content="NOREYO – Travel made for you. Finde Hotels und Reisen, die wirklich zu deinen Wünschen passen.">');
  return html;
}
function stripAsset(html,kind,name){
  const tag=kind==='style'?'link':'script',attr=kind==='style'?'href':'src';
  const re=new RegExp('<'+tag+'\\b[^>]*'+attr+'=["\\\']\\./'+escRe(name)+'(?:\\?[^"\\\']*)?["\\\'][^>]*>'+(kind==='script'?'\\s*</script>':''),'gi');
...
function patch(html){
  html=html.split('V5.27').join(BUILD).split('content="5.27"').join('content="'+BUILD+'"').split('BUILD 5.27').join('BUILD '+BUILD);
  html=brand(html);
  if(!html.includes('</style>'))throw new Error('Style-Anker fehlt');
  html=html.replace('</style>',HOTFIX+'</style>');
  for(const [name] of STYLE_ASSETS)html=stripAsset(html,'style',name);
  for(const [name] of SCRIPT_ASSETS)html=stripAsset(html,'script',name);
  if(!html.includes('</head>')||!html.includes('</body>'))throw new Error('HTML-Anker fehlen');
  html=html.replace('</head>',STYLE_ASSETS.map(styleTag).join('')+'</head>');
  return html.replace('</body>',SCRIPT_ASSETS.map(scriptTag).join('')+'</body>');
}
...
async function boot(){
  try{
    ...
    const r=await fetchWithTimeout('./site.zip?build='+SITE_BUILD,{cache:'no-store'},30000);
    if(!r.ok)throw new Error('site.zip konnte nicht geladen werden ('+r.status+')');
    const buffer=await r.arrayBuffer();let files;
    try{files=await unzipNative(buffer);}catch(error){console.warn(error);files=await unzipFallback(buffer);}
    let html=patch(new TextDecoder().decode(files['index.html']));
    setStatus('Bilder werden vorbereitet …');
    for(const [name,bytes] of Object.entries(files)){
      if(!name.startsWith('assets/')||name.endsWith('/'))continue;
      html=html.split(name).join(URL.createObjectURL(new Blob([bytes],{type:mime(name)})));
    }
