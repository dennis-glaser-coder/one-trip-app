/* NOREYO V8.55 — favorite fallback stale-state repair.
   Keeps V6.70 fallback cards synchronized with savedFavorites after remove/error
   paths, including the previously uncovered transition to an empty list. */
(function(){
'use strict';
const BUILD='8.55';
let observer=null,raf=0,repairing=false;
...
function renderedKeys(listEl=document.getElementById('favList')){const cards=fallbackCards(listEl);if(!cards.length)return[];return cards.map(x=>key(x.getAttribute('data-v584-open')));}
...
function bind(){document.addEventListener('click',onClick,true);observe();schedule();return true;}
function cleanup(){document.removeEventListener('click',onClick,true);if(observer){observer.disconnect();observer=null;}if(raf){cancelAnimationFrame(raf);raf=0;}repairing=false;}
bind();window.addEventListener('pagehide',cleanup,{passive:true});window.addEventListener('pageshow',bind,{passive:true});window.NOREYO_V855=Object.freeze({BUILD,saved,key,savedKeys,fallbackCards,renderedKeys,sameSet,repair,schedule,relevantClick,observe,bind,cleanup});
})();