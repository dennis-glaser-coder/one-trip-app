/* NOREYO V8.33 bootstrap arbiter — isolated retries plus final BFCache fetch-stack stabilization. */
(()=>{
'use strict';
const BUILD='8.33-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000;
const KEY='__NOREYO_V833_SINGLE_FLIGHT__';
...
function retrySrc(src,attempt){
  return attempt<=1?src:src+(src.includes('?')?'&':'?')+'noreyo_v833_retry='+attempt;
}
function fail(error){
...
window.NOREYO_V833=Object.freeze({BUILD,ATTEMPTS,RETRY_MS,TIMEOUT_MS,KEY,COMPONENTS,state,retrySrc,loadOnce,loadComponent,run});
run().catch(()=>{});
})();