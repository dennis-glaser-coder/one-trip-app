/* NOREYO V9.38 bootstrap — iPhone gallery lifecycle + truthful hotel/flight package planning. */
(()=>{
'use strict';
const BUILD='9.38-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,READY_TIMEOUT_MS=45000,POLL_MS=25,KEY='__NOREYO_V938_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  {name:'V9.35',src:'./noreyo-bootstrap-v935.js?build=935',asyncReady:true,ready:()=>window.NOREYO_V935?.state?.().status==='ready'},
  {name:'Gallery modal V9.36',src:'./noreyo-v936.js?build=936',ready:()=>window.NOREYO_V936?.BUILD==='9.36'},
  {name:'Package truth V9.37',src:'./noreyo-v937.js?build=937',ready:()=>window.NOREYO_V937?.BUILD==='9.37'}
]);
...
function src(b,a){return a<=1?b:b+(b.includes('?')?'&':'?')+'noreyo_v938_retry='+a}
function sleep(ms){return new Promise(r=>setTimeout(r,ms))}
async function waitReady(c){if(c.ready())return true;const start=Date.now();while(Date.now()-start<READY_TIMEOUT_MS){await sleep(POLL_MS);if(c.ready())return true;if(c.name==='V9.35'){const i=window.NOREYO_V935?.state?.();if(i?.status==='failed')throw Object.assign(i.error||new Error('V9.35 inner failed'),{replaySafe:false})}}return false}
...
window.NOREYO_V938=Object.freeze({BUILD,COMPONENTS,state,run});run().catch(()=>{})
})();