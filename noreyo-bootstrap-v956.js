/* NOREYO V9.56 bootstrap — V9.54 plus capability-truth copy. */
(()=>{
'use strict';
const BUILD='9.56-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,READY_TIMEOUT_MS=45000,POLL_MS=25,KEY='__NOREYO_V956_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  {name:'V9.54',src:'./noreyo-bootstrap-v954.js?build=954',asyncReady:true,ready:()=>window.NOREYO_V954?.state?.().status==='ready'},
  {name:'Capability truth V9.55',src:'./noreyo-v955.js?build=955',ready:()=>window.NOREYO_V955?.BUILD==='9.55'}
]);
...
function src(b,a){return a<=1?b:b+(b.includes('?')?'&':'?')+'noreyo_v956_retry='+a}
function sleep(ms){return new Promise(r=>setTimeout(r,ms))}
async function waitReady(c){if(c.ready())return true;const start=Date.now();while(Date.now()-start<READY_TIMEOUT_MS){await sleep(POLL_MS);if(c.ready())return true;if(c.name==='V9.54'){const i=window.NOREYO_V954?.state?.();if(i?.status==='failed')throw Object.assign(i.error||new Error('V9.54 inner failed'),{replaySafe:false})}}return false}
...
window.NOREYO_V956=Object.freeze({BUILD,COMPONENTS,state,run});run().catch(()=>{})
})();