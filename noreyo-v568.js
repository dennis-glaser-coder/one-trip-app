(function(){
  ...
  function currentProductMode(){
    try{if(typeof productMode!=='undefined'&&typeof productMode==='string')return productMode;}catch{}
  ...
  function ensureErrorHost(root){
    let host=root.querySelector('.noreyo-v568-error');if(host)return host;
    host=document.createElement('div');host.className='noreyo-v568-error';host.setAttribute('role','alert');host.setAttribute('aria-live','assertive');
    ...
    const save=root.querySelector('.planner-save');if(save)save.insertAdjacentElement('beforebegin',host);else root.appendChild(host);return host;
  }

  function block(event,root,message,focus){
    event.preventDefault();event.stopImmediatePropagation();const host=ensureErrorHost(root);host.textContent=message;focus?.focus?.({preventScroll:false});
  ...
  function showToastAndOpen(message,planner){
    try{
      if(typeof toast==='function')toast(message);else if(typeof showToast==='function')showToast(message);else alert(message);
  ...
  function guard(event){
    ...
    if(target.classList.contains('planner-save')){
      const modal=target.closest('.planner-sheet,.sheet,[role="dialog"]');if(!modal)return;
      modal.querySelector('.noreyo-v568-error')?.remove();const kind=plannerKind(modal);
      const result=kind==='airports'?validateAirportPlanner(modal):kind==='destination'?validateDestinationPlanner(modal):{ok:true};
      if(!result.ok)block(event,modal,result.message,result.focus);return;
    }
    if(target.classList.contains('liveSearchButton')||target.classList.contains('noreyo-v541-booking-cta')){
      ...
      if(!airports.ok){event.preventDefault();event.stopImmediatePropagation();showToastAndOpen(airports.message,'airports');}
    }
  }

  function cleanupTransientErrors(){document.querySelectorAll('.noreyo-v568-error').forEach(el=>el.remove());}
  function install(){document.addEventListener('click',guard,true);window.addEventListener('pagehide',cleanupTransientErrors,{passive:true});}

  window.NOREYO_V568=Object.freeze({currentProductMode,validateAirportPlanner,validateDestinationPlanner,validateVisibleAirports,parseAirportSummary});
