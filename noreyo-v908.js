...
function observe(){if(observer){observer.disconnect();observer=null;}const el=root();if(typeof MutationObserver==='undefined'||!el)return false;/* Treat an already-open gallery as a fresh open so BFCache/hot-load still receives dialog semantics and focus. */lastOpen=false;observer=new MutationObserver(schedule);observer.observe(el,{attributes:true,attributeFilter:['class']});schedule();return true;}
