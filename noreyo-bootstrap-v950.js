/* NOREYO V9.50 bootstrap — V9.48 plus actionable expired-flight recovery. */
(()=>{
'use strict';
const BUILD='9.50-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,READY_TIMEOUT_MS=45000,POLL_MS=25,KEY='__NOREYO_V950_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  {name:'V9.48',src:'./noreyo-bootstrap-v948.js?build=948',asyncReady:true,ready:()=>window.NOREYO_V948?.state?.().status==='ready'},
  {name:'Expired flight retry V9.49',src:'./noreyo-v949.js?build=949',ready:()=>window.NOREYO_V949?.BUILD==='9.49'}
]);
...
function src(b,a){return a<=1?b:b+(b.includes('?')?'&':'?')+'noreyo_v950_retry='+a}
function sleep(ms){return new Promise(r=>setTimeout(r,ms))}
async function waitReady(c){if(c.ready())return true;const start=Date.now();while(Date.now()-start<READY_TIMEOUT_MS){await sleep(POLL_MS);if(c.ready())return true;if(c.name==='V9.48'){const i=window.NOREYO_V948?.state?.();if(i?.status==='failed')throw Object.assign(i.error||new Error('V9.48 inner failed'),{replaySafe:false})}}return false}
...
window.NOREYO_V950=Object.freeze({BUILD,COMPONENTS,state,run});run().catch(()=>{})
})();