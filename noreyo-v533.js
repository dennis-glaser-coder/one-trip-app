(function(){
  ...
  function preferenceCounts(){return{must:Object.values(states).filter(x=>x==='must').length,wish:Object.values(states).filter(x=>x==='wish').length};}
  function decorateSearch(){
    document.querySelectorAll('#discover .search-card,#searchView .search-card').forEach(card=>{
      const head=card.querySelector('.search-console-head'),old=card.querySelector('.noreyo-search-logic');
      if(productMode==='flight'||!head){old?.remove();return;}if(old)return;
  ...
  function decorateResultPrinciple(){
    const anchor=document.querySelector('#results .match');if(!anchor)return;const counts=preferenceCounts();let box=document.querySelector('#results .noreyo-results-principle');
    if(!box){anchor.insertAdjacentHTML('afterend','<div class="noreyo-results-principle"></div>');box=document.querySelector('#results .noreyo-results-principle');}
  ...
  function decorateOffers(){
    document.querySelectorAll('#offers .offer').forEach(card=>{
      card.classList.add('noreyo-offer');
      const img=card.querySelector('.offer-img');
      if(img){
        let badge=img.querySelector('.noreyo-match-badge');
        if(!badge){img.insertAdjacentHTML('beforeend','<div class="noreyo-match-badge"><span>NOREYO MATCH</span><strong>WIRD GEPRÜFT</strong><small>Hotelmerkmale werden sicher zugeordnet</small></div>');badge=img.querySelector('.noreyo-match-badge');}
        if(badge)badge.dataset.noreyoNeutral='1';
      }
      const line=card.querySelector('.offer-matchline');if(line&&!line.children.length)line.innerHTML='<span class="offer-matchchip neutral">Tarifdaten werden geprüft</span>';
      card.querySelector('.offer-essentials')?.classList.add('noreyo-essentials');
      card.querySelector('.offer-price-context')?.classList.add('noreyo-price-panel');
      const btn=card.querySelector('.dark-btn');
      if(btn){
        btn.classList.add('noreyo-cta');btn.innerHTML=`Hotel & Tarife ansehen ${svg('chev')}`;
        let trust=card.querySelector('.noreyo-trust-row');
        if(!trust){btn.insertAdjacentHTML('beforebegin','<div class="noreyo-trust-row"><span><i></i>Tarifdaten verfügbar</span></div>');trust=card.querySelector('.noreyo-trust-row');}
        if(trust)trust.dataset.noreyoNeutral='1';
      }
    });
  ...
  function detailMatchCopy(m){
    ...
    const must=m.items.filter(x=>x.state==='must'),wishOk=m.items.filter(x=>x.state==='wish'&&x.ok).length,mustOk=must.filter(x=>x.ok).length;
  ...
  function decorateDetail(o){
    const root=document.getElementById('detailContent');if(!root)return;root.querySelector('.noreyo-detail-match')?.remove();const rating=root.querySelector('.detail-rating');if(!rating)return;
