/* NOREYO V7.63 — corrected BFCache hot-function stack stabilizer. */
(function(){
'use strict';
const BUILD='7.63';
const SPECS=[{name:'filterAndRankOffers',markers:['__noreyoV677','__noreyoV706']},{name:'persistState',markers:['__noreyoV688','__noreyoV751']},{name:'toggleFav',markers:['__noreyoV606','__noreyoV703Id']},{name:'toggleSnapshotFavorite',markers:['__noreyoV606','__noreyoV703Snapshot']}];
...
function installOne(spec){const current=getFn(spec.name),finalMarker='__noreyoV763_'+spec.name;if(typeof current!=='function')return false;if(current[finalMarker]){mark(current,spec.markers,finalMarker);return false;}const wrapped=function(){return current.apply(this,arguments);};mark(wrapped,spec.markers,finalMarker);return setFn(spec.name,wrapped);}
...
window.NOREYO_V763=Object.freeze({BUILD,SPECS:SPECS.map(x=>({name:x.name,markers:x.markers.slice()})),getFn,installOne,install,markers});
})();