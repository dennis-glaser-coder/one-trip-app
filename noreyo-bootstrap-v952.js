/* NOREYO V9.52 bootstrap — V9.50 plus persistent truthful result sorting. */
(()=>{
'use strict';
const BUILD='9.52-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,READY_TIMEOUT_MS=45000,POLL_MS=25,KEY='__NOREYO_V952_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  {name:'V9.50',src:'./noreyo-bootstrap-v950.js?build=950',asyncReady:true,ready:()=>window.NOREYO_V950?.state?.().status==='ready'},
  {name:'Sort consistency V9.51',src:'./noreyo-v951.js?build=951',ready:()=>window.NOREYO_V951?.BUILD==='9.51'}
]);
...
function src(b,a){return a<=1?b:b+(b.includes('?')?'&':'?')+'noreyo_v952_retry='+a}
function sleep(ms){return new Promise(r=>setTimeout(r,ms))}
async function waitReady(c){if(c.ready())return true;const start=Date.now();while(Date.now()-start<READY_TIMEOUT_MS){await sleep(POLL_MS);if(c.ready())return true;if(c.name==='V9.50'){const i=window.NOREYO_V950?.state?.();if(i?.status==='failed')throw Object.assign(i.error||new Error('V9.50 inner failed'),{replaySafe:false})}}return false}
...
window.NOREYO_V952=Object.freeze({BUILD,COMPONENTS,state,run});run().catch(()=>{})
})();