/* NOREYO V9.46 bootstrap — selectable flight offers plus expiry safety. */
(()=>{
'use strict';
const BUILD='9.46-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,READY_TIMEOUT_MS=45000,POLL_MS=25,KEY='__NOREYO_V946_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  {name:'V9.44',src:'./noreyo-bootstrap-v944.js?build=944',asyncReady:true,ready:()=>window.NOREYO_V944?.state?.().status==='ready'},
  {name:'Flight expiry safety V9.45',src:'./noreyo-v945.js?build=945',ready:()=>window.NOREYO_V945?.BUILD==='9.45'}
]);
...
function src(b,a){return a<=1?b:b+(b.includes('?')?'&':'?')+'noreyo_v946_retry='+a}
function sleep(ms){return new Promise(r=>setTimeout(r,ms))}
async function waitReady(c){if(c.ready())return true;const start=Date.now();while(Date.now()-start<READY_TIMEOUT_MS){await sleep(POLL_MS);if(c.ready())return true;if(c.name==='V9.44'){const i=window.NOREYO_V944?.state?.();if(i?.status==='failed')throw Object.assign(i.error||new Error('V9.44 inner failed'),{replaySafe:false})}}return false}
...
window.NOREYO_V946=Object.freeze({BUILD,COMPONENTS,state,run});run().catch(()=>{})
})();