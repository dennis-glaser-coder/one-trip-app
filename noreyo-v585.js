...
function currentTravellersValid(){
  ...
  return Number.isInteger(adults)&&adults>=1&&adults<=9&&adults+ages.length<=9&&ages.every(v=>Number.isInteger(v)&&v>=0&&v<=17)&&ages.filter(v=>v<=1).length<=adults;
}
function openTravellers(){
  try{if(typeof openPlanner==='function'){travellerPlannerRequested=true;openPlanner('travellers');return true;}}catch(_){}
  const candidates=['.travellerInput','.travelerInput','[data-planner="travellers"]','[data-field="travellers"]'];
  for(const q of candidates){const el=document.querySelector(q);if(el){travellerPlannerRequested=true;el.click();return true;}}
  return false;
}
function notify(message){
  try{if(typeof showToast==='function')showToast(message);else if(typeof toast==='function')toast(message);else alert(message);}catch(_){}
}
...
function onPlannerSave(event){
  ...
  if(!save)return;
  setTimeout(()=>{if(currentTravellersValid()){pendingFamily=null;travellerPlannerRequested=false;}},0);
}
function install(){
  ...
  document.addEventListener('keydown',e=>{if((e.metaKey||e.ctrlKey)&&e.key==='Enter'&&e.target?.id==='noreyoAi556Text'){pendingFamily=null;setTimeout(decorate,0);}},true);
}
window.NOREYO_V585=Object.freeze({BUILD,parseAdults,noChildren,childCount,parseAges,parseFamily,applyFamily,decorate,currentTravellersValid,clearPending(){pendingFamily=null;travellerPlannerRequested=false;},get pendingFamily(){return pendingFamily;}});
