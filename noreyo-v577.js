/* NOREYO V5.97 — premium discovery + isolated safety loader */
(()=>{
'use strict';
...
function hero(){const layer=document.querySelector('#discover .noreyo-v574-hero-layer');if(!layer||layer.dataset.v577==='1')return;layer.dataset.v577='1';layer.innerHTML='<div class="noreyo-v577-badge">NOREYO</div><div class="noreyo-v577-hero-copy"><small>REISEN, DIE ZU DIR PASSEN</small><h1>Finde deinen Urlaub.<br><b>Auf deine Art.</b></h1><p>Klassisch suchen – oder NOREYO einfach sagen, was dir wichtig ist.</p></div>';}
function smart(){const host=document.querySelector('#discover [data-v574-usp]');if(!host||host.dataset.v577==='1')return;host.dataset.v577='1';host.classList.add('noreyo-v577-smart');const ph=examples[mode()]||examples.package;host.innerHTML='<div class="noreyo-v577-dock"><button type="button" data-v577-open="text"><span>✦</span><b>Wunsch beschreiben</b><small>Ein Satz reicht</small></button><button type="button" data-v577-open="photos"><span>▧</span><b>Bilder zeigen</b><small>Inspiration ergänzen</small></button></div><div class="noreyo-v577-panel"><div class="noreyo-v577-panel-head"><div><small>NOREYO SUCHE</small><b>Was ist dir im Urlaub wichtig?</b></div><button type="button" data-v577-close aria-label="Schließen">×</button></div><textarea data-v574-text maxlength="500" rows="2" aria-label="Urlaub beschreiben" placeholder="'+ph+'"></textarea><div class="noreyo-v577-panel-actions"><label><input type="file" accept="image/*" multiple data-v574-photos><span>▧</span><b>Bilder hinzufügen</b></label><button type="button" data-v574-go>Wunsch übernehmen <span>→</span></button></div><div class="noreyo-v574-preview" data-v574-preview></div><p class="noreyo-v577-photo-note">Bilder können bereits ausgewählt werden; die automatische Bildanalyse ist noch nicht aktiv.</p></div>';}
function cleanup(){document.querySelectorAll('#discover .noreyo-v574-mode-intro').forEach(x=>x.style.display='none');const card=document.querySelector('#discover .search-card');if(card)card.classList.add('noreyo-v577-search-card');const products=document.querySelector('#discover .product-switch.noreyo-v552-products');if(products)products.classList.add('noreyo-v577-products');}
...
function loadCss(){if(document.querySelector('link[data-noreyo-v593]'))return;const l=document.createElement('link');l.rel='stylesheet';l.href='./noreyo-v593.css?build=597';l.dataset.noreyoV593='1';l.onload=()=>retries.delete('593');l.onerror=()=>{l.remove();retry('593',loadCss);};document.head.appendChild(l);}
function loadScript(key,globalName){if(window[globalName]||document.querySelector(`script[data-noreyo-v${key}]`))return;const s=document.createElement('script');s.src=`./noreyo-v${key}.js?build=597`;s.async=false;s.dataset[`noreyoV${key}`]='1';s.onload=()=>retries.delete(key);s.onerror=()=>{s.remove();retry(key,()=>loadScript(key,globalName));};document.head.appendChild(s);}
...
document.addEventListener('click',e=>{const open=e.target.closest?.('[data-v577-open]');if(open){e.preventDefault();openPanel(open.dataset.v577Open);return;}if(e.target.closest?.('[data-v577-close]')){e.preventDefault();closePanel();return;}if(e.target.closest?.('#discover .product-mode'))setTimeout(updatePlaceholder,180);});
