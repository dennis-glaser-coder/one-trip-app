(function(){
  ...
  function productMode(){
    try{return typeof window.productMode==='string'?window.productMode:(typeof globalThis.productMode==='string'?globalThis.productMode:'package');}
  ...
  function ensureErrorHost(root){
    let host=root.querySelector('.noreyo-v567-error');
    if(host)return host;
    host=document.createElement('div');
    host.className='noreyo-v567-error';
  ...
  function validateAirportPlanner(modal){
    if(productMode()==='hotel')return {ok:true};
  ...
  function showToastAndOpen(message,planner){
    try{
      ...
      if(planner&&typeof window.openPlanner==='function')setTimeout(()=>window.openPlanner(planner),0);
    }catch{alert(message);}
  }
  ...
  function guard(event){
    ...
    if(target.classList.contains('planner-save')){
      ...
      if(!modal)return;
      modal.querySelector('.noreyo-v567-error')?.remove();
  ...
  function cleanupTransientErrors(){
    document.querySelectorAll('.noreyo-v567-error').forEach(el=>el.remove());
  }
  ...
  window.NOREYO_V567=Object.freeze({
