/* NOREYO V5.85 — provider occupancy alignment + safety guard loaders */
(function(){
'use strict';
const BUILD='5.85';
const MAX_ADULTS=6;
const MAX_TRAVELLERS=9;
let v582Attempts=0,v582Timer=0,v585Attempts=0,v585Timer=0;
...
function loadGuard(key,globalName,src,attemptKey){
 ...
 let attempts=is582?v582Attempts:v585Attempts;
 if(window[globalName]){if(is582){v582Attempts=0;clearTimeout(v582Timer);v582Timer=0;}else{v585Attempts=0;clearTimeout(v585Timer);v585Timer=0;}return;}
 if(document.querySelector(`script[data-noreyo-v${key}]`)||attempts>=3)return;
 attempts++;
 if(is582)v582Attempts=attempts;else v585Attempts=attempts;
 const s=document.createElement('script');s.src=src;s.dataset[`noreyoV${key}`]='1';
 s.onload=()=>{if(is582){v582Attempts=0;clearTimeout(v582Timer);v582Timer=0;}else{v585Attempts=0;clearTimeout(v585Timer);v585Timer=0;}};
 s.onerror=()=>{s.remove();const current=is582?v582Attempts:v585Attempts;if(current<3){const delay=350*Math.pow(2,current-1);if(is582){clearTimeout(v582Timer);v582Timer=setTimeout(loadV582,delay);}else{clearTimeout(v585Timer);v585Timer=setTimeout(loadV585,delay);}}};
 document.head.appendChild(s);
}
function loadV582(){loadGuard('582','NOREYO_V582','./noreyo-v582.js?build=585','582');}
function loadV585(){loadGuard('585','NOREYO_V585','./noreyo-v585.js?build=585','585');}
function cleanup(){clearTimeout(v582Timer);clearTimeout(v585Timer);v582Timer=0;v585Timer=0;}
function install(){document.addEventListener('click',onSearch,true);syncBuildLabel();loadV582();loadV585();if(typeof MutationObserver!=='undefined'){const mo=new MutationObserver(records=>{if(relevant(records))syncBuildLabel();});mo.observe(document.body,{childList:true,subtree:true});}window.addEventListener('pageshow',()=>{syncBuildLabel();loadV582();loadV585();},{passive:true});window.addEventListener('pagehide',cleanup,{passive:true});}
window.NOREYO_V581=Object.freeze({BUILD,MAX_ADULTS,MAX_TRAVELLERS,occupancyError,syncBuildLabel,loadV582,loadV585});
