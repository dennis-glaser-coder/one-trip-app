/* NOREYO V8.27 bootstrap arbiter — single-flight V8.24 delivery plus final request completeness. */
(()=>{
'use strict';
const BUILD='8.27-safe',INNER='./noreyo-bootstrap-v824.js?build=824',COMPLETENESS='./noreyo-v826.js?build=826',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000;
const KEY='__NOREYO_V827_SINGLE_FLIGHT__';
...
function retrySrc(src,attempt){
  return attempt<=1?src:src+(src.includes('?')?'&':'?')+'noreyo_v827_retry='+attempt;
}
function fail(error){
...
window.NOREYO_V827=Object.freeze({BUILD,INNER,COMPLETENESS,ATTEMPTS,RETRY_MS,TIMEOUT_MS,KEY,state,retrySrc,loadOnce,loadCompleteness,run});
run().catch(()=>{});
})();