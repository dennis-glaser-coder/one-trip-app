/* NOREYO V9.48 bootstrap — V9.46 plus truthful local price-marker copy. */
(()=>{
'use strict';
const BUILD='9.48-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,READY_TIMEOUT_MS=45000,POLL_MS=25,KEY='__NOREYO_V948_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  {name:'V9.46',src:'./noreyo-bootstrap-v946.js?build=946',asyncReady:true,ready:()=>window.NOREYO_V946?.state?.().status==='ready'},
  {name:'Price marker truth V9.47',src:'./noreyo-v947.js?build=947',ready:()=>window.NOREYO_V947?.BUILD==='9.47'}
]);
...
function src(b,a){return a<=1?b:b+(b.includes('?')?'&':'?')+'noreyo_v948_retry='+a}
function sleep(ms){return new Promise(r=>setTimeout(r,ms))}
async function waitReady(c){if(c.ready())return true;const start=Date.now();while(Date.now()-start<READY_TIMEOUT_MS){await sleep(POLL_MS);if(c.ready())return true;if(c.name==='V9.46'){const i=window.NOREYO_V946?.state?.();if(i?.status==='failed')throw Object.assign(i.error||new Error('V9.46 inner failed'),{replaySafe:false})}}return false}
...
window.NOREYO_V948=Object.freeze({BUILD,COMPONENTS,state,run});run().catch(()=>{})
})();