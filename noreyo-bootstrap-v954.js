/* NOREYO V9.54 bootstrap — V9.52 plus saved-price freshness truth. */
(()=>{
'use strict';
const BUILD='9.54-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,READY_TIMEOUT_MS=45000,POLL_MS=25,KEY='__NOREYO_V954_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  {name:'V9.52',src:'./noreyo-bootstrap-v952.js?build=952',asyncReady:true,ready:()=>window.NOREYO_V952?.state?.().status==='ready'},
  {name:'Saved price truth V9.53',src:'./noreyo-v953.js?build=953',ready:()=>window.NOREYO_V953?.BUILD==='9.53'}
]);
...
function src(b,a){return a<=1?b:b+(b.includes('?')?'&':'?')+'noreyo_v954_retry='+a}
function sleep(ms){return new Promise(r=>setTimeout(r,ms))}
async function waitReady(c){if(c.ready())return true;const start=Date.now();while(Date.now()-start<READY_TIMEOUT_MS){await sleep(POLL_MS);if(c.ready())return true;if(c.name==='V9.52'){const i=window.NOREYO_V952?.state?.();if(i?.status==='failed')throw Object.assign(i.error||new Error('V9.52 inner failed'),{replaySafe:false})}}return false}
...
window.NOREYO_V954=Object.freeze({BUILD,COMPONENTS,state,run});run().catch(()=>{})
})();