...
function explicitAdultIntent(text){
  const t=norm(text);
...
function restoreAdults(expected){
  const s=state();
  if(!s||!Number.isInteger(expected)||expected<1||expected>9)return false;
...
function onApplyCapture(event){
  ...
  if(explicitAdultIntent(text))return;
