/* NOREYO V6.98 bootstrap — final request integrity plus current preference/viewport/result safety. */
(()=>{
'use strict';
const BUILD='6.98-safe';
const SITE_BUILD='noreyo-698';
const HOTFIX=`
...
const SCRIPT_ASSETS=[
 ...
 ['noreyo-v673.js',690],['noreyo-v677.js',677],['noreyo-v679.js',684],['noreyo-v681.js',681],['noreyo-v683.js',686],['noreyo-v688.js',688],['noreyo-v692.js',692],['noreyo-v693.js',693],['noreyo-v694.js',694],['noreyo-v695.js',695],['noreyo-v696.js',696],['noreyo-v697.js',697],['noreyo-v698.js',698]
];
...
function findEocd(u8,dv){const min=Math.max(0,u8.length-0xFFFF-22);for(let off=u8.length-22;off>=min;off--){if(dv.getUint32(off,true)===0x06054b50)return off;}return-1;}
async function unzipNative(buffer){
  const u8=new Uint8Array(buffer),dv=new DataView(buffer),td=new TextDecoder();
  const eocd=findEocd(u8,dv);if(eocd<0)throw new Error('ZIP central directory fehlt');
  const entries=dv.getUint16(eocd+10,true),centralSize=dv.getUint32(eocd+12,true),centralOffset=dv.getUint32(eocd+16,true);
  if(entries===0xFFFF||centralSize===0xFFFFFFFF||centralOffset===0xFFFFFFFF)throw new Error('ZIP64 wird nicht unterstützt');
  if(centralOffset+centralSize>u8.length)throw new Error('ZIP central directory beschädigt');
  const out={};let off=centralOffset,seen=0;
  while(seen<entries){
    if(off+46>u8.length||dv.getUint32(off,true)!==0x02014b50)throw new Error('ZIP central entry beschädigt');
    const flags=dv.getUint16(off+8,true),method=dv.getUint16(off+10,true),csize=dv.getUint32(off+20,true),usize=dv.getUint32(off+24,true),nlen=dv.getUint16(off+28,true),xlen=dv.getUint16(off+30,true),clen=dv.getUint16(off+32,true),localOffset=dv.getUint32(off+42,true);
    if(flags&0x1)throw new Error('Verschlüsselte ZIP-Dateien werden nicht unterstützt');
    const nameStart=off+46,nameEnd=nameStart+nlen;if(nameEnd>u8.length)throw new Error('ZIP Dateiname beschädigt');
    const name=td.decode(u8.subarray(nameStart,nameEnd));
    if(localOffset+30>u8.length||dv.getUint32(localOffset,true)!==0x04034b50)throw new Error('ZIP local header beschädigt');
    const localNlen=dv.getUint16(localOffset+26,true),localXlen=dv.getUint16(localOffset+28,true),start=localOffset+30+localNlen+localXlen,end=start+csize;
    if(end>u8.length)throw new Error('ZIP Datei beschädigt: '+name);
    const comp=u8.slice(start,end);let raw;
    if(method===0)raw=comp;else if(method===8)raw=await inflateRaw(comp);else throw new Error('ZIP compression '+method+' unsupported');
    if(usize!==0xFFFFFFFF&&raw.length!==usize)throw new Error('ZIP Größe stimmt nicht: '+name);
    out[name]=raw;off=nameEnd+xlen+clen;seen++;
  }
  if(!out['index.html'])throw new Error('index.html fehlt im Paket');return out;
}
...
window.NOREYO_BOOTSTRAP=Object.freeze({BUILD,SITE_BUILD,patch,unzipNative,findEocd,styles:STYLE_ASSETS,scripts:SCRIPT_ASSETS});
boot();
})();
