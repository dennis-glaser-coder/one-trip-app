/* NOREYO V8.80 consolidated bootstrap — V8.64 plus current safety layers without wrapper-chain startup hops. */
(()=>{
'use strict';
const BUILD='8.80-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,READY_TIMEOUT_MS=45000,POLL_MS=25;
const KEY='__NOREYO_V880_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  ...
  {name:'Abflug Latest Intent V8.70',src:'./noreyo-v870.js?build=870',ready:()=>window.NOREYO_V870?.BUILD==='8.70'},
  {name:'Familien-Payload V8.72',src:'./noreyo-v872.js?build=872',ready:()=>window.NOREYO_V872?.BUILD==='8.72'},
  {name:'Fetch-Stabilisierer V8.74',src:'./noreyo-v874.js?build=874',ready:()=>window.NOREYO_V874?.BUILD==='8.74'},
  {name:'Zeitraum Latest Intent V8.76',src:'./noreyo-v876.js?build=876',ready:()=>window.NOREYO_V876?.BUILD==='8.76'},
  {name:'Reisende Latest Intent V8.78',src:'./noreyo-v878.js?build=878',ready:()=>window.NOREYO_V878?.BUILD==='8.78'}
]);
...
function src(base,attempt){return attempt<=1?base:base+(base.includes('?')?'&':'?')+'noreyo_v880_retry='+attempt;}
function sleep(ms){return new Promise(r=>setTimeout(r,ms));}
async function waitReady(component){if(component.ready())return true;const start=Date.now();while(Date.now()-start<READY_TIMEOUT_MS){await sleep(POLL_MS);if(component.ready())return true;if(component.name==='V8.64'){const inner=window.NOREYO_V864?.state?.();if(inner?.status==='failed')throw Object.assign(inner.error||new Error('V8.64 inner failed'),{replaySafe:false});}}return false;}
...
window.NOREYO_V880=Object.freeze({BUILD,ATTEMPTS,RETRY_MS,TIMEOUT_MS,READY_TIMEOUT_MS,POLL_MS,KEY,COMPONENTS,state,src,sleep,waitReady,loadOnce,loadComponent,run});run().catch(()=>{});
})();