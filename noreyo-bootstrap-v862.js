/* NOREYO V8.62 bootstrap arbiter — complete German natural dates plus BFCache-safe favorite reconciliation. */
(()=>{
'use strict';
const BUILD='8.62-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,INNER_READY_TIMEOUT_MS=45000,READY_POLL_MS=25;
const KEY='__NOREYO_V862_SINGLE_FLIGHT__';
...
function retrySrc(src,attempt){return attempt<=1?src:src+(src.includes('?')?'&':'?')+'noreyo_v862_retry='+attempt;}
function fail(error){const bar=document.getElementById('bar'),status=document.getElementById('status'),box=document.getElementById('error');if(bar)bar.style.display='none';if(status)status.textContent='NOREYO konnte nicht geladen werden';if(box){box.style.display='block';box.setAttribute('role','alert');box.setAttribute('aria-live','assertive');if(!box.querySelector('[data-noreyo-v862-retry="1"]')){box.textContent='Die Verbindung zum Reisemodul ist fehlgeschlagen.';const br=document.createElement('br'),button=document.createElement('button');button.type='button';button.className='boot-retry';button.setAttribute('data-noreyo-v862-retry','1');button.textContent='Erneut versuchen';button.addEventListener('click',()=>location.reload());box.appendChild(br);box.appendChild(button);}}console.error(error);}
...
window.NOREYO_V862=Object.freeze({BUILD,ATTEMPTS,RETRY_MS,TIMEOUT_MS,INNER_READY_TIMEOUT_MS,READY_POLL_MS,KEY,COMPONENTS,state,retrySrc,sleep,waitReady,loadOnce,loadComponent,run});
run().catch(()=>{});
})();