/* NOREYO V9.42 bootstrap — V9.40 plus multi-origin live flight search. */
(()=>{
'use strict';
const BUILD='9.42-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,READY_TIMEOUT_MS=45000,POLL_MS=25,KEY='__NOREYO_V942_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  {name:'V9.40',src:'./noreyo-bootstrap-v940.js?build=940',asyncReady:true,ready:()=>window.NOREYO_V940?.state?.().status==='ready'},
  {name:'Multi-origin flight V9.41',src:'./noreyo-v941.js?build=941',ready:()=>window.NOREYO_V941?.BUILD==='9.41'}
]);
...
function src(b,a){return a<=1?b:b+(b.includes('?')?'&':'?')+'noreyo_v942_retry='+a}
function sleep(ms){return new Promise(r=>setTimeout(r,ms))}
async function waitReady(c){if(c.ready())return true;const start=Date.now();while(Date.now()-start<READY_TIMEOUT_MS){await sleep(POLL_MS);if(c.ready())return true;if(c.name==='V9.40'){const i=window.NOREYO_V940?.state?.();if(i?.status==='failed')throw Object.assign(i.error||new Error('V9.40 inner failed'),{replaySafe:false})}}return false}
...
window.NOREYO_V942=Object.freeze({BUILD,COMPONENTS,state,run});run().catch(()=>{})
})();