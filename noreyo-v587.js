/* NOREYO V5.87 — unified early search preflight for native + live CTA */
(function(){
'use strict';
const BUILD='5.87';
...
function searchTarget(target){
 ...
 const btn=target.closest('.noreyo-v541-booking-cta,.liveSearchButton');if(!btn)return null;
 return{btn,managedByV577:btn.matches('.liveSearchButton')};
}
...
function ensureFamilySafety(){
 ...
 safetyAttempts++;
 const s=document.createElement('script');s.src='./noreyo-v577.js?build=587';s.dataset.noreyoV577='1';
...
function plannerKindFor(error){
 const t=norm(error);if(t.includes('reiseziel'))return'destination';if(t.includes('zeitraum')||t.includes('abreise')||t.includes('rueckreise'))return'dates';if(t.includes('abflughafen'))return'airports';return'travellers';
}
...
function onClick(e){
 const target=searchTarget(e.target);if(!target)return;
 ...
 if(target.managedByV577)return;
 beginBusy(target.btn);
}
...
function install(){ensureFamilySafety();bindResults();attachRootObserver();window.addEventListener('click',onClick,true);window.addEventListener('pagehide',cleanup,{passive:true});window.addEventListener('pageshow',restore,{passive:true});}
window.NOREYO_V587=Object.freeze({BUILD,validateSearchSafety,coreSearchError,validISO,airportsValid,ensureFamilySafety,releaseBusy,get busy(){return busy;}});
