...
function searchTarget(target){if(!(target instanceof Element))return null;const btn=target.closest('.noreyo-v541-booking-cta,.liveSearchButton');if(!btn)return null;return{btn,managedByV577:btn.matches('.liveSearchButton')};}
...
function scheduleProviderRetry(){if(window.NOREYO_V581||providerAttempts>=3)return;clearTimeout(providerTimer);providerTimer=setTimeout(ensureProviderSafety,350*Math.pow(2,Math.max(0,providerAttempts-1)));}
function ensureProviderSafety(){
 if(window.NOREYO_V581){providerAttempts=0;clearTimeout(providerTimer);providerTimer=0;return;}
...
function mode(){const active=document.querySelector('#discover .product-mode.on');const t=norm(active?.textContent||'');if(t.includes('kreuzfahrt'))return'cruise';if(t.includes('hotel'))return'hotel';if(t.includes('flug'))return'flight';try{if(typeof productMode==='string'&&productMode)return productMode;}catch(_){ }return'package';}
...
function validateSearchSafety(){
 ...
 if(!window.NOREYO_V581){ensureProviderSafety();return{error:'Belegungsprüfung wird geladen. Bitte gleich noch einmal suchen.',unavailable:true,planner:''};}
 try{
   api.applyFamily();
   ...
   ensureFamilySafety();ensureProviderSafety();
   return{error:'Suchprüfung konnte noch nicht abgeschlossen werden. Bitte erneut versuchen.',unavailable:true,planner:''};
 }
}
function onClick(e){const target=searchTarget(e.target);if(!target)return;const check=validateSearchSafety();if(check.error){e.preventDefault();e.stopImmediatePropagation();notify(check.error);if(check.planner)setTimeout(()=>openPlannerFor(check.planner),0);return;}if(window.NOREYO_V577?.busy||busy){e.preventDefault();e.stopImmediatePropagation();return;}if(target.managedByV577)return;beginBusy(target.btn);}
...
function cleanup(){releaseBusy();clearTimeout(safetyTimer);clearTimeout(providerTimer);safetyTimer=0;providerTimer=0;if(observer){observer.disconnect();observer=null;}if(rootObserver){rootObserver.disconnect();rootObserver=null;}observedRoot=null;}
function restore(){releaseBusy();ensureFamilySafety();ensureProviderSafety();bindResults();attachRootObserver();}
function install(){ensureFamilySafety();ensureProviderSafety();bindResults();attachRootObserver();window.addEventListener('click',onClick,true);window.addEventListener('pagehide',cleanup,{passive:true});window.addEventListener('pageshow',restore,{passive:true});}
window.NOREYO_V587=Object.freeze({BUILD,validateSearchSafety,coreSearchError,validISO,airportsValid,ensureFamilySafety,ensureProviderSafety,releaseBusy,get busy(){return busy;}});
