/* NOREYO V9.40 bootstrap — V9.38 plus iPhone 44px touch-target floor. */
(()=>{
'use strict';
const BUILD='9.40-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,READY_TIMEOUT_MS=45000,POLL_MS=25,KEY='__NOREYO_V940_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  {name:'V9.38',src:'./noreyo-bootstrap-v938.js?build=938',asyncReady:true,ready:()=>window.NOREYO_V938?.state?.().status==='ready'},
  {name:'Touch targets V9.39',src:'./noreyo-v939.js?build=939',ready:()=>window.NOREYO_V939?.BUILD==='9.39'}
]);
...
function src(b,a){return a<=1?b:b+(b.includes('?')?'&':'?')+'noreyo_v940_retry='+a}
function sleep(ms){return new Promise(r=>setTimeout(r,ms))}
async function waitReady(c){if(c.ready())return true;const start=Date.now();while(Date.now()-start<READY_TIMEOUT_MS){await sleep(POLL_MS);if(c.ready())return true;if(c.name==='V9.38'){const i=window.NOREYO_V938?.state?.();if(i?.status==='failed')throw Object.assign(i.error||new Error('V9.38 inner failed'),{replaySafe:false})}}return false}
...
window.NOREYO_V940=Object.freeze({BUILD,COMPONENTS,state,run});run().catch(()=>{})
})();