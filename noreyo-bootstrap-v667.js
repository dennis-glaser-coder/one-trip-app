/* NOREYO V6.67 bootstrap — versioned outer loader. */
(()=>{
'use strict';
const BUILD='6.67-safe';
const HOTFIX=`
...
function stripAsset(html,kind,name){const attr=kind==='style'?'href':'src',tag=kind==='style'?'link':'script';const re=new RegExp('<'+tag+'\\b[^>]*'+attr+'=["\\\']\\./'+reEsc(name)+'(?:\\?[^"\\\']*)?["\\\'][^>]*>'+(kind==='script'?'\\s*<\\/script>':''),'gi');return html.replace(re,'');}
const styles=[
 ['noreyo-v533.css',582,'<link rel="stylesheet" href="./noreyo-v533.css?build=582">'],
 ['noreyo-v534.css',582,'<link rel="stylesheet" data-noreyo-v534="1" href="./noreyo-v534.css?build=582">'],
 ['noreyo-v541.css',582,'<link rel="stylesheet" href="./noreyo-v541.css?build=582">'],
 ['noreyo-v544.css',657,'<link rel="stylesheet" href="./noreyo-v544.css?build=657">'],
 ['noreyo-v551.css',582,'<link rel="stylesheet" href="./noreyo-v551.css?build=582">'],
 ['noreyo-v552.css',582,'<link rel="stylesheet" href="./noreyo-v552.css?build=582">'],
 ['noreyo-v555.css',582,'<link rel="stylesheet" href="./noreyo-v555.css?build=582">'],
 ['noreyo-v556.css',582,'<link rel="stylesheet" href="./noreyo-v556.css?build=582">'],
 ['noreyo-v557.css',582,'<link rel="stylesheet" href="./noreyo-v557.css?build=582">'],
 ['noreyo-v558.css',582,'<link rel="stylesheet" href="./noreyo-v558.css?build=582">'],
 ['noreyo-v583.css',583,'<link rel="stylesheet" href="./noreyo-v583.css?build=583">'],
 ['noreyo-v584.css',584,'<link rel="stylesheet" href="./noreyo-v584.css?build=584">'],
 ['noreyo-v585.css',619,'<link rel="stylesheet" href="./noreyo-v585.css?build=619">']
];
const scripts=[
 ...
 ['noreyo-v553.js',661],['noreyo-v554.js',662],['noreyo-v555.js',582],['noreyo-v556.js',587],
 ['noreyo-v557.js',667],['noreyo-v558.js',658],['noreyo-v584.js',589],['noreyo-v606.js',627],
 ['noreyo-v585.js',602],['noreyo-v607.js',656],['noreyo-v612.js',630],['noreyo-v591.js',635],
 ['noreyo-v624.js',652],['noreyo-v636.js',660],['noreyo-v644.js',659]
];
function patch(html){
 html=html.split('V5.27').join(BUILD).split('content="5.27"').join('content="'+BUILD+'"').split('BUILD 5.27').join('BUILD '+BUILD);
 html=brand(html);
 const end='</style>';if(!html.includes(end))throw new Error('Style-Anker fehlt');
 let patched=html.replace(end,HOTFIX+end);
 styles.forEach(([name])=>{patched=stripAsset(patched,'style',name);});
 patched=patched.replace('</head>',styles.map(([, ,tag])=>tag).join('')+'</head>');
 scripts.forEach(([name])=>{patched=stripAsset(patched,'script',name);});
 const enhancer=scripts.map(([name,build])=>'<scr'+'ipt src="./'+name+'?build='+build+'"></scr'+'ipt>').join('');
 return patched.replace('</body>',enhancer+'</body>');
}
async function boot(){
 try{
  setStatus('NOREYO '+BUILD+' wird vorbereitet …');
  const r=await fetch('./site.zip?build=noreyo-667',{cache:'no-store'});
  if(!r.ok)throw new Error('site.zip konnte nicht geladen werden ('+r.status+')');
  const buffer=await r.arrayBuffer();let files;
  try{files=await unzipNative(buffer)}catch(e){console.warn(e);files=await unzipFallback(buffer)}
  let html=patch(new TextDecoder().decode(files['index.html']));
  setStatus('Bilder werden vorbereitet …');
  for(const [name,bytes] of Object.entries(files)){if(!name.startsWith('assets/')||name.endsWith('/'))continue;const url=URL.createObjectURL(new Blob([bytes],{type:mime(name)}));html=html.split(name).join(url)}
  document.open();document.write(html);document.close();
 }catch(err){
  console.error(err);document.getElementById('bar')?.style&&(document.getElementById('bar').style.display='none');
...
window.NOREYO_BOOTSTRAP=Object.freeze({BUILD,patch,styles,scripts});
boot();
})();