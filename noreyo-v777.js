/* NOREYO V7.77 — total-party traveller semantics.
   Corrects natural phrases where "Personen" or "zu fünft" is explicitly the total party
   so children are not added on top of that total. Explicit adult wording always wins. */
(function(){
'use strict';
const BUILD='7.77';
const words={eins:1,eine:1,einer:1,einem:1,ein:1,zwei:2,zweit:2,drei:3,dritt:3,vier:4,viert:4,fuenf:5,funf:5,fuenft:5,funft:5,sechs:6,sechst:6,sieben:7,siebt:7,acht:8,neun:9,neunt:9};
...
function explicitAdults(text){const t=norm(text);const m=t.match(/\b([1-9]|ein|eins|eine|einer|einem|zwei|drei|vier|fuenf|funf|sechs)\s+(?:erwachsene|erwachsenen)\b/);return m?number(m[1]):null;}
function childCount(text){const t=norm(text);if(/\b(?:keine|kein|ohne)\s+(?:kinder|kind)\b/.test(t))return 0;let m=t.match(/\b([1-4]|ein|eine|einen|einem|einer|zwei|drei|vier)\s+(?:kinder|kindern|kind)\b/);if(m)return number(m[1]);try{const ages=window.NOREYO_V747?.groupedChildAges?.(text);if(Array.isArray(ages))return ages.length;}catch(_){}return null;}
...
window.NOREYO_V777=Object.freeze({BUILD,norm,number,explicitAdults,childCount,totalPersons,derive,applyDerived});
})();