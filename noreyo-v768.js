...
(function(){
'use strict';
const BUILD='7.68',DEFAULT_NIGHTS=7,MAX_PRESERVED_NIGHTS=60;
...
function repairPlan(s){if(!s)return null;const today=todayISO();if(validISO(s.checkin)&&s.checkin>=today&&validISO(s.checkout)&&s.checkout>s.checkin)return null;const oldNights=nights(s.checkin,s.checkout),staleOrInvalidCheckin=!validISO(s.checkin)||String(s.checkin)<today;let checkin=s.checkin,checkout=s.checkout;if(staleOrInvalidCheckin)checkin=tomorrowISO();if(!validISO(checkout)||checkout<=checkin||staleOrInvalidCheckin)checkout=addDays(checkin,oldNights||DEFAULT_NIGHTS);if(!validISO(checkout)||checkout<=checkin)checkout=addDays(checkin,DEFAULT_NIGHTS);return{checkin,checkout,nights:oldNights||DEFAULT_NIGHTS,preserved:oldNights!==null};}
...
window.NOREYO_V768=Object.freeze({BUILD,DEFAULT_NIGHTS,MAX_PRESERVED_NIGHTS,validISO,isoDate,todayISO,tomorrowISO,addDays,nights,snapshot,repairPlan,repair});
})();