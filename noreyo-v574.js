/* NOREYO V5.74 — first-screen USP before classic search */
(()=>{
'use strict';
let painting=false;
let photoUrls=[];
...
function heroMarkup(){return '<div class="noreyo-v574-hero-layer">'+
 '<div class="noreyo-v574-hero-badge">NOREYO</div>'+
 '<div class="noreyo-v574-hero-copy"><small>DIE REISESUCHE, DIE DICH VERSTEHT</small><h1>Du kennst das Gefühl.<br><b>NOREYO findet die Reise dazu.</b></h1><p>Beschreib deinen Wunsch, zeig Bilder, die dir gefallen – oder such ganz klassisch.</p></div>'+
 '<div class="noreyo-v574-hero-paths"><span>✦ Beschreiben</span><span>▧ Bilder zeigen</span><span>⌕ Klassisch</span></div>'+
 '</div>';}
function uspMarkup(){const p=hasProfile();return '<section class="noreyo-v574-usp" data-v574-usp>'+
 '<div class="noreyo-v574-usp-head"><div><small>DEIN WUNSCH. DEINE BILDER. EINE SUCHE.</small><h2>Zeig NOREYO, wie dein Urlaub aussehen soll.</h2></div>'+(p?'<em>Vorlieben aktiv</em>':'')+'</div>'+
 '<p>Du musst keinen Filternamen kennen. Schreib, was dir wichtig ist, und ergänze 3–8 Fotos oder Screenshots, die zeigen, was dir gefällt.</p>'+
 '<div class="noreyo-v574-compose"><textarea data-v574-text maxlength="500" rows="2" aria-label="Urlaub beschreiben"></textarea><div class="noreyo-v574-actions">'+
 '<label class="noreyo-v574-images"><input type="file" accept="image/*" multiple data-v574-photos><span>▧</span><b>3–8 Bilder zeigen</b><small>Fotos oder Screenshots</small><i>+</i></label>'+
 '<button type="button" data-v574-go>Diesen Urlaub finden <span>→</span></button></div><div class="noreyo-v574-preview" data-v574-preview></div></div>'+
 '<div class="noreyo-v574-formula"><small>NOREYO VERBINDET</small><div><span>deinen Wunsch</span><b>+</b><span>deine Bilder</span>'+(p?'<b>+</b><span>deine Vorlieben</span>':'')+'<i>→</i><strong>deine Reisesuche</strong></div></div>'+
 '</section>';}
...
function showPhotos(files){clearPhotos();const arr=[...(files||[])].slice(0,8);photoUrls=arr.map(f=>URL.createObjectURL(f));const host=document.querySelector('#discover [data-v574-preview]');if(!host)return;if(!arr.length){host.innerHTML='';return;}host.innerHTML='<div class="noreyo-v574-thumbs">'+photoUrls.slice(0,5).map((u,i)=>'<img src="'+u+'" alt="Ausgewähltes Urlaubsbild '+(i+1)+'">').join('')+(arr.length>5?'<span>+'+(arr.length-5)+'</span>':'')+'</div><div><b>'+arr.length+' Bild'+(arr.length===1?'':'er')+' gewählt</b><small>'+(arr.length<3?'Mehrere Bilder zeigen deinen gewünschten Stil besser.':'Mehrere Eindrücke ergeben ein klareres Bild deines Urlaubs.')+'</small><em>Bildanalyse noch nicht aktiv</em></div>';}
