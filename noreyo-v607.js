/* NOREYO V6.55 — authoritative search completion/network lifecycle guard.
   Active package/hotel requests never unlock solely because of the UI timer;
   stalled search-travel calls are aborted and parallel failures aggregate safely. */
(function(){
'use strict';
const BUILD='6.55';
...
function bindButtonObserver(){
  ...
  buttonObserver=new MutationObserver(enforceButtonLock);buttonObserver.observe(guardButton,{attributes:true,attributeFilter:['disabled','aria-disabled']});
}
...
function onGuardTimeout(){
  guardTimer=0;if(!guardActive)return;
  if(targetRequests>0){guardTimer=setTimeout(onGuardTimeout,5000);return;}
  releaseGuard('timeout-idle');
}
function startGuard(btn){
  if(guardActive)return false;
  guardActive=true;guardButton=btn||null;resetCompletionState();bindButtonObserver();scheduleButtonLock(guardButton);
  clearTimeout(guardTimer);guardTimer=setTimeout(onGuardTimeout,16000);return true;
}
function releaseGuard(reason){
  ...
  if(buttonObserver){buttonObserver.disconnect();buttonObserver=null;}
  if(guardButton){guardButton.disabled=false;guardButton.removeAttribute('aria-disabled');}
...
function onSearchCapture(e){
  ...
  if(guardActive){e.preventDefault();e.stopImmediatePropagation();enforceButtonLock();return;}
  startGuard(btn);
}
function terminalCompletionReady(){
  ...
  const results=document.getElementById('results');return !!results&&terminalText(results.textContent||'');
}
...
function bind(){
  const next=document.getElementById('results');if(next===root&&observer)return;
  if(observer){observer.disconnect();observer=null;}root=next;if(!root||typeof MutationObserver==='undefined')return;
  observer=new MutationObserver(records=>{if(records?.length)resultSequence++;scheduleTerminal();});
...
function completeAfterNetwork(){
  if(!guardActive||targetRequests>0)return false;
  if(networkSuccess){if(completionReady())return releaseGuard('rendered-after-network');scheduleTerminal();return false;}
  if(networkFailures>0)return releaseGuard('all-network-requests-failed');
  return false;
}
function timedRequestInit(init){
  if(typeof AbortController==='undefined')return {init,cleanup(){}};
  const controller=new AbortController(),upstream=init?.signal;let upstreamAbort=null;
  if(upstream){
  ...
  return {
    ...
    cleanup(){clearTimeout(timer);if(upstream&&upstreamAbort)upstream.removeEventListener?.('abort',upstreamAbort);}
  };
}
function installFetchHook(){
  try{
    if(typeof window.fetch!=='function'||window.fetch.__noreyoV655)return;
    const prior=window.fetch.bind(window);
    const wrapped=async function(input,init){
      const target=isSearchTravel(input)&&guardActive;
      let timed=null,requestInit=init;
      if(target){targetRequests++;timed=timedRequestInit(init);requestInit=timed.init;}
      try{
    ...
    wrapped.__noreyoV655=true;window.fetch=wrapped;
  }catch(_){ }
}
function installRenderHook(){
  try{
    if(typeof renderOffers!=='function'||renderOffers.__noreyoV655)return;
    const prior=renderOffers;
    const wrapped=function(){
      let result;try{result=prior.apply(this,arguments);}catch(error){releaseGuard('render-error');throw error;}
      const done=()=>{renderSequence++;completeAfterNetwork();};const failed=()=>{releaseGuard('render-rejected');};
      if(result&&typeof result.then==='function')result.then(done,failed);else setTimeout(done,0);return result;
    };
    wrapped.__noreyoV655=true;renderOffers=wrapped;
  }catch(_){ }
}
function installNavigationHook(){
  try{
    if(typeof go!=='function'||go.__noreyoV655)return;
    const prior=go;
    ...
    wrapped.__noreyoV655=true;go=wrapped;
  }catch(_){ }
}
function install(){
  bind();installFetchHook();installRenderHook();installNavigationHook();
  if(!window.__noreyoV655SearchCapture){window.__noreyoV655SearchCapture=true;window.addEventListener('click',onSearchCapture,true);}
}
function cleanup(){
  releaseGuard('pagehide');clearTimeout(lockTimer);lockTimer=0;
  if(observer){observer.disconnect();observer=null;}if(raf){cancelAnimationFrame(raf);raf=0;}root=null;
}
...
window.NOREYO_V607=Object.freeze({
  ...
  releaseTerminalIfVisible,completeAfterNetwork,timedRequestInit,bind,startGuard,releaseGuard,onGuardTimeout,
