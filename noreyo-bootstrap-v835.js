/* NOREYO V8.35 bootstrap arbiter — true inner readiness without unsafe replay.
   V8.24 is ready only after its final injected layer (V7.84) exists. A loaded
   asynchronous inner bootstrap is never replayed merely because readiness is delayed. */
(()=>{
'use strict';
const BUILD='8.35-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,INNER_READY_TIMEOUT_MS=45000,READY_POLL_MS=25;
const KEY='__NOREYO_V835_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  ...
  {name:'fetch-stack',src:'./noreyo-v832.js?build=832',ready:()=>window.NOREYO_V832?.BUILD==='8.32'}
]);
...
function retrySrc(src,attempt){
  return attempt<=1?src:src+(src.includes('?')?'&':'?')+'noreyo_v835_retry='+attempt;
}
function fail(error){
  ...
  if(box){
    ...
    if(!box.querySelector('[data-noreyo-v835-retry="1"]')){
      ...
      button.type='button';button.className='boot-retry';button.setAttribute('data-noreyo-v835-retry','1');
...
async function waitReady(component,timeoutMs){
...
function loadOnce(component,attempt){
  return new Promise((resolve,reject)=>{
    ...
    const script=document.createElement('script');let settled=false,timer=0,loaded=false;
    const finish=(ok,error)=>{
      if(settled)return;settled=true;
      if(timer)clearTimeout(timer);
      script.onload=script.onerror=null;
      if(!ok&&!loaded){try{script.remove();}catch(_){} }
      ok?resolve(true):reject(error||new Error(component.name+' konnte nicht geladen werden'));
    };
    timer=setTimeout(()=>finish(false,Object.assign(new Error(component.name+' Netzwerk-/Script-Timeout'),{replaySafe:!loaded})),TIMEOUT_MS);
    script.src=retrySrc(component.src,attempt);
    script.onload=async()=>{
      loaded=true;
      if(timer){clearTimeout(timer);timer=0;}
      if(!component.asyncReady)return finish(component.ready(),Object.assign(new Error(component.name+' meldet keinen gültigen Startzustand'),{replaySafe:true}));
      const ready=await waitReady(component,INNER_READY_TIMEOUT_MS);
      if(ready)return finish(true);
      finish(false,Object.assign(new Error(component.name+' wurde geladen, aber der App-Stack wurde nicht vollständig bereit'),{replaySafe:false}));
    };
    script.onerror=()=>finish(false,Object.assign(new Error(component.name+' konnte nicht geladen werden'),{replaySafe:true}));
    document.head.appendChild(script);
  });
}
async function loadComponent(component,s=state()){
  if(component.ready())return true;
  let last=null;s.component=component.name;
  for(let attempt=1;attempt<=ATTEMPTS;attempt++){
    s.attempt=attempt;
    try{return await loadOnce(component,attempt);}
    catch(error){
      last=error;
      if(error?.replaySafe===false||attempt>=ATTEMPTS)break;
...
window.NOREYO_V835=Object.freeze({BUILD,ATTEMPTS,RETRY_MS,TIMEOUT_MS,INNER_READY_TIMEOUT_MS,READY_POLL_MS,KEY,COMPONENTS,state,retrySrc,sleep,waitReady,loadOnce,loadComponent,run});
run().catch(()=>{});
})();