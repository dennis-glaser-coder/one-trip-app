/* NOREYO V8.64 bootstrap arbiter — complete German dates plus safe departure-context parsing. */
(()=>{
'use strict';
const BUILD='8.64-safe',ATTEMPTS=2,RETRY_MS=260,TIMEOUT_MS=15000,INNER_READY_TIMEOUT_MS=45000,READY_POLL_MS=25;
const KEY='__NOREYO_V864_SINGLE_FLIGHT__';
const COMPONENTS=Object.freeze([
  ...
  {name:'date-elided-range',src:'./noreyo-v859.js?build=859',ready:()=>window.NOREYO_V859?.BUILD==='8.59'},
  {name:'date-natural-single',src:'./noreyo-v853.js?build=853',ready:()=>window.NOREYO_V853?.BUILD==='8.53'},
  {name:'date-am-single',src:'./noreyo-v861.js?build=861',ready:()=>window.NOREYO_V861?.BUILD==='8.61'},
  {name:'airport-natural',src:'./noreyo-v851.js?build=851',ready:()=>window.NOREYO_V851?.BUILD==='8.51'},
  {name:'airport-context',src:'./noreyo-v863.js?build=863',ready:()=>window.NOREYO_V863?.BUILD==='8.63'},
  {name:'favorites-stale',src:'./noreyo-v857.js?build=857',ready:()=>window.NOREYO_V857?.BUILD==='8.57'},
...
function retrySrc(src,attempt){return attempt<=1?src:src+(src.includes('?')?'&':'?')+'noreyo_v864_retry='+attempt;}
function fail(error){const bar=document.getElementById('bar'),status=document.getElementById('status'),box=document.getElementById('error');if(bar)bar.style.display='none';if(status)status.textContent='NOREYO konnte nicht geladen werden';if(box){box.style.display='block';box.setAttribute('role','alert');box.setAttribute('aria-live','assertive');if(!box.querySelector('[data-noreyo-v864-retry="1"]')){box.textContent='Die Verbindung zum Reisemodul ist fehlgeschlagen.';const br=document.createElement('br'),button=document.createElement('button');button.type='button';button.className='boot-retry';button.setAttribute('data-noreyo-v864-retry','1');button.textContent='Erneut versuchen';button.addEventListener('click',()=>location.reload());box.appendChild(br);box.appendChild(button);}}console.error(error);}
...
window.NOREYO_V864=Object.freeze({BUILD,ATTEMPTS,RETRY_MS,TIMEOUT_MS,INNER_READY_TIMEOUT_MS,READY_POLL_MS,KEY,COMPONENTS,state,retrySrc,sleep,waitReady,loadOnce,loadComponent,run});
run().catch(()=>{});
})();