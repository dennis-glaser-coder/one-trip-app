/* NOREYO V5.84 — fail-closed native CTA safety + BFCache-safe observers */
(function(){
'use strict';
const BUILD='5.84';
...
function validateFamily(){
 ...
 try{api.applyFamily();const error=api.validateBeforeSearch();return{error:error||'',unavailable:false};}
 catch(_){ensureFamilySafety();return{error:'Reisendenprüfung konnte noch nicht abgeschlossen werden. Bitte erneut versuchen.',unavailable:true};}
}
function onClick(e){
 const btn=nativeCTA(e.target);if(!btn)return;
 const check=validateFamily();
 if(check.error){e.preventDefault();e.stopImmediatePropagation();notify(check.error);if(!check.unavailable)setTimeout(openTravellers,0);return;}
