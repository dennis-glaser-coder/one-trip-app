/* NOREYO V5.88 — profile + redundant safety/preflight loader */
(()=>{
'use strict';
...
function markup(){
 ...
 return '<div class="noreyo-v576-wrap">'
  ...
  +'<div class="noreyo-v576-build">NOREYO · BUILD 5.88</div>'
  +'</div>';
}
function install(){
...
function scheduleCssRetry(){if(document.querySelector('link[data-noreyo-v578]')||cssRetries>=3)return;clearTimeout(cssRetryTimer);cssRetries++;cssRetryTimer=setTimeout(loadSafetyCss,500*Math.pow(2,cssRetries-1));}
...
function loadSafetyCss(){
 ...
 const l=document.createElement('link');l.rel='stylesheet';l.href='./noreyo-v578.css?build=588';l.dataset.noreyoV578='1';
...
function loadPreflightJs(){
 ...
 if(document.querySelector('script[data-noreyo-v587]'))return;
 const s=document.createElement('script');s.src='./noreyo-v587.js?build=588';s.dataset.noreyoV587='1';
...
window.addEventListener('pagehide',()=>{clearTimeout(jsRetryTimer);clearTimeout(cssRetryTimer);clearTimeout(preflightRetryTimer);},{passive:true});
