/* NOREYO V7.47 — natural grouped child-age parsing with duplicate preservation.
   Supports phrases such as "2 Kinder, 5 und 8 Jahre" and twins such as
   "2 Kinder, 5 und 5 Jahre" without mistaking trip duration for ages. */
(function(){
'use strict';
const BUILD='7.47';
...
function sharedYears(segment){const out=[],re=/((?:\b\d{1,2}\b\s*(?:,\s*|\bund\b\s*)){1,3}\b\d{1,2}\b)\s*(?:jahre?|j\.)\b/g;let m;while((m=re.exec(segment))){const nums=m[1].match(/\d{1,2}/g)||[];nums.forEach(x=>addAge(out,Number(x)));}return out;}
function sharedMonths(segment){const out=[],re=/((?:\b\d{1,2}\b\s*(?:,\s*|\bund\b\s*)){1,3}\b\d{1,2}\b)\s*monate?\b/g;let m;while((m=re.exec(segment))){const nums=m[1].match(/\d{1,2}/g)||[];nums.forEach(x=>{const n=Number(x);if(Number.isInteger(n)&&n>=0&&n<=23)addAge(out,Math.floor(n/12));});}return out;}
...
window.NOREYO_V747=Object.freeze({BUILD,norm,wordNumber,explicitChildCount,childSegment,sharedYears,sharedMonths,individuallyQualified,groupedChildAges,reconcile});
})();