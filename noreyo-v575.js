/* NOREYO V5.75 — one first-screen experience: USP + input + classic visible */
(()=>{
'use strict';
let painting=false;
...
function tuneHero(){const layer=document.querySelector('#discover .noreyo-v574-hero-layer');if(!layer)return;if(layer.dataset.v575==='1')return;layer.dataset.v575='1';layer.innerHTML=heroMarkup();}
function tuneUsp(){const usp=document.querySelector('#discover [data-v574-usp]');if(!usp)return;if(usp.dataset.v575==='1')return;usp.dataset.v575='1';usp.classList.add('noreyo-v575-usp');usp.innerHTML=quickMarkup();}
function tuneClassic(){const intro=document.querySelector('#discover .noreyo-v574-mode-intro');if(intro)intro.innerHTML='<small>ODER KLASSISCH SUCHEN</small>';}
